...
data.sort_values(data.columns[0], ascending=True, inplace=True)
...
# cleaned data
clean_headlines = [clean(i) for i in headlines]
clean_headlines = list(map(lambda x: x.split(), clean_headlines))
...
doc_term_matrix = [dictionary.doc2bow(i) for i in clean_headlines]

# build LDA model
lda_model = gensim.models.LdaModel(doc_term_matrix, num_topics=5, id2word = dictionary, random_state=2, iterations=10)
...
def compute_coherence_values(dictionary, corpus, texts, limit, start=2, step=3):
    ...
    for i in range(start, limit, step):
        ...
        num_topics=i, random_state=2, id2word=dictionary, iterations=10)
...
model_list, coherence_values = compute_coherence_values(dictionary=dictionary,
     corpus=doc_term_matrix, texts=clean_headlines, limit=50, start=2, step=6)
